...
\title{Genesis Formalization}
\author{IOHK \and Well-Typed LLP}
\maketitle

\begin{abstract}
We present a psi-calculus formalization, embedded in Haskell, of the Genesis blockchain protocol \cite{genesis}.
\end{abstract}
...
data UnsignedBlock = Block {
     blockState  :: Maybe (Hash Block)  -- $h$: the hash of the last block or Nothing if first block.
  ,  blockSlot   :: SlotNumber          -- sl
  ,  blockData   :: [Transaction]       -- st
  ,  blockProof  :: VrfProof            -- $crt = (y,\pi)$ but here we omit $y$ (why?)
